    ...
    <table class="table">
        ...
        @foreach ($latestAttendancesAndOut as $row)
          <tr>
            ...
            <td>{{ $row->name }}</td>
            <td>{{ \Carbon\Carbon::parse($row->latest_attendance)->format('D, M d, Y')}} - {{ \Carbon\Carbon::parse($row->latest_attendance)->format('h:i A') }} - 
              @if($row->attendance_status == 1)
              <span class="badge bg-success">On-Time</span>
              @elseif($row->attendance_status == 2)
              <span class="badge bg-danger">Late</span>
              @endif
            <td>{{ \Carbon\Carbon::parse($row->latest_out)->format('D, M d, Y')}} - {{ \Carbon\Carbon::parse($row->latest_out)->format('h:i A') }}</td>
            {{-- <td>{{ $row->attendanceStatus ?? 'Null'}}</td> --}}
            <td><a href="/hr/{{ $row->user_id}}/attendance-log" class="btn btn-light btn-outline-dark py-0" style="font-weight:100;">View Logs</a>
            </td>
          </tr>
        @endforeach
        </tbody>
